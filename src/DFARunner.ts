...
export default class DFARunner {
    ...
    hasConcluded() {
        return this.status == DFARunnerStatus.Accepted || this.status == DFARunnerStatus.Rejected;
    }
    ...
    runStep() {
        ...
        for (let runInstanceIdx = 0; runInstanceIdx < this.runInstances.length; runInstanceIdx++) {
            ...
            for (let transitionIdx = 0; transitionIdx < this.dfa.transitions.length; transitionIdx++) {
                ...
                if (runInstance.currentState != transition.currentState || transition.inputToken != currentToken) {
                    continue;
                }

                const newInstance = new DFARunInstance(transition.targetState, runInstance.remainingTokens.slice(1));
