...
export default class DFA {
    ...
    verifyTransitions(): Array<BaseAutomatonError> {
        let errors = Array<BaseAutomatonError>();
        ...
        for (let stateIdx = 0; stateIdx < this.states.length; stateIdx++) {
            const transitionsForCurrentState = this.transitionsForCurrentState(stateIdx)

            for (let tkIdx = 0; tkIdx < this.inputAlphabet.length; tkIdx++) {
                const token = this.inputAlphabet[tkIdx];
                const numTransWithToken = transitionsForCurrentState.filter((t) => t.inputToken == token).length;
                if (numTransWithToken != 1) {
                    errors.push(new AutomatonStateWrongNumTransitionsError(this.states[stateIdx], numTransWithToken, token));
                }
            }
        }
